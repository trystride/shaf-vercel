...
export async function sendMatchNotifications(matches: MatchWithDetails[]) {
  ...
  const userMatches = matches.reduce((acc, match) => {
...
async function sendEmailNotification(user: User, matches: MatchWithDetails[]) {
  const matchesByKeyword = matches.reduce((acc, match) => {
  ...
  try {
    ...
    console.error('Error sending email via Plunk:', error.response?.data || error.message);
    throw error;
  }
}

function generateEmailContent(matchesByKeyword: Record<string, { keyword: Keyword; announcements: Announcement[] }>) {
  let content = `
